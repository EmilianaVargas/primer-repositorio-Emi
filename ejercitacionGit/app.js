...
function cargarCartelera(){
    ...
    const totalPeliCartelera = '\nTotal de películas en cartelera​: ​(' + movies.length + ')';
    const listadoPeliCartelera = '\n\nListados de películas:'
    const pelisMasReseña = movies.map(movie =>"\n\nTítulo: " + movie.title + "\n Reseña: "+ movie.overview);
    return tituloCartelera + totalPeliCartelera + listadoPeliCartelera + pelisMasReseña;
}

function cargarMasVotadas(){
	const tituloMasVotadas = 'Más Votadas.\n';
	const voteAverageMayor7 = movies.filter(movie => movie.vote_average >= 7);
	const totalPeliMasVotadas = '\n\n Total de películas mas votadas​: ' + voteAverageMayor7.length;

	let sumaRatings = 0;
	voteAverageMayor7.forEach(elem => sumaRatings += elem.vote_average);

	const ratingPromedio = "\n\n Rating promedio: " + Math.round(sumaRatings/voteAverageMayor7.length);
    const listadoPeliMasVotadas = '\n\nListados de películas:';
    const pelisMasVotadas = voteAverageMayor7.map(movie =>"\n\n Título: " + movie.title + "\n Rating: " + movie.vote_average + "\n Reseña: "+ movie.overview);
    return tituloMasVotadas + totalPeliMasVotadas + ratingPromedio + listadoPeliMasVotadas + pelisMasVotadas;
}
...
function cargarContacto(){
    ...
    return tituloContacto + contenidoContacto;
}

function cargarPreguntasFrecuentes(){
    const tituloPreguntasFrecuentes = 'Preguntas Frecuentes.\n';
    const totalPreguntas = '\nTotal de preguntas​: ​(' + faqs.length + ')';
    const listadoPreguntas = '\n\nListados de preguntas:'
    const porCadaPregunta = faqs.map(faqs =>"\n\n Pregunta: " +faqs.faq_title + "\n Respuesta: " + faqs.faq_answer);
    return tituloPreguntasFrecuentes+totalPreguntas+listadoPreguntas+porCadaPregunta;
}
...
http.createServer((req, res) => {
	...
	switch (req.url) {
		...
		// En cartelera
		case '/en-cartelera':
			let detalleCartelera = cargarCartelera();
            res.write(detalleCartelera);
			res.end();
			break;
		...
		case '/contacto':
			let detalleContacto = cargarContacto();
			res.write(detalleContacto);
			res.end();
			break;


		case '/preguntas-frecuentes':
			let detalleFaqs = cargarPreguntasFrecuentes();
			res.write(detalleFaqs)
			res.end();
			break;
